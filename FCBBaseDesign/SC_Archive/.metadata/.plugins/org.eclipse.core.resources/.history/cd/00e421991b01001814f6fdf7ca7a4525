...
void MCP3903ResetOSR(byte osr)
{
	...
	MCP3903WriteRegister(REG_CONFIG, cmd1);
	MCP3903WriteRegister(REG_CONFIG, cmd2);
}
...
unsigned long MCP3903ReadRegister(byte reg)
{
	...
	bufffer = SPI_transfer_frame(this_spi,cmdByte); //Push data out (register), dummy read in but unused, command byte
...
{
	unsigned long r = MCP3903ReadRegister(channel);
...
{
	MCP3903SetGainCB(channel, gain, 0);
}
...
{
	unsigned long r = MCP3903ReadRegister(REG_GAIN);
	...
	MCP3903WriteRegister(REG_GAIN, r);
}
